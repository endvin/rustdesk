...
**第一步：查看错误**
1. 点击失败的工作流运行
2. 点击 "build-android-arm64-only" 作业
3. 查看红色的步骤
4. 展开查看详细错误日志
...
| 错误 | 解决方案 |
|------|---------|
| vcpkg 下载失败 | 点击 "Re-run failed jobs" 重试 |
| 磁盘空间不足 | 已自动清理，重试即可 |
| Rust 编译错误 | 检查 `src/bridge_generated.rs` 是否已提交 |
| Flutter 构建错误 | 检查 `flutter/lib/generated_bridge.dart` 是否已提交 |
| NDK 未找到 | 工作流会自动安装，重试即可 |
...
查看完整的故障排查指南：[TROUBLESHOOTING.md](TROUBLESHOOTING.md)
